#!/usr/bin/env python3
"""
gen-agents-local.py — Generate AGENTS.md navigation for a directory.
...
The source-hash is a SHA1 of sorted immediate child names and regular-file
sizes. It is a cheap routing hint, not proof of unchanged source content.
"""
...
SKIP_FILES = {"AGENTS.md", "AGENTS.local.md", "OUTDATED.local.md", ".DS_Store"}

OUTPUT_FILENAME = "AGENTS.md"
GENERATED_MARKER = "<!-- AGENTS.md: generated navigation; keep this file checked in -->"
MAX_FILES_TO_READ = 5


def included(item: Path) -> bool:
    return (not item.is_symlink() and not item.name.startswith(".")
            and item.name not in SKIP_FILES
            and (item.name not in SKIP_DIRS or
                 (item.is_dir() and (item / "build.gradle.kts").is_file())))
...
def source_hash(directory: Path) -> str:
    ...
    for item in sorted(directory.iterdir()):
        if included(item):
            entries.append(f"{item.name}:{item.stat().st_size}" if item.is_file() else item.name)
    return hashlib.sha1("\n".join(entries).encode()).hexdigest()[:12]


def existing_hash(agent_guide: Path) -> str | None:
    if not agent_guide.exists():
        return None
    with agent_guide.open() as f:
        for line in f:
...
def subdirs(directory: Path) -> list[Path]:
    ...
    try:
        for item in sorted(directory.iterdir()):
            if item.is_dir() and included(item):
                result.append(item)
    except PermissionError:
        pass
    return result
...
def subdir_desc(subdir: Path) -> str:
    # Use existing generated AGENTS.md purpose line if available
...
def render(directory: Path, hash_val: str) -> str:
    ...
    if dirs:
        ...
        for d in dirs:
            nav_path = f"{d.name}/{OUTPUT_FILENAME}" if (d / OUTPUT_FILENAME).is_file() else f"{d.name}/"
            desc = subdir_desc(d)
    ...
    if (parent / OUTPUT_FILENAME).is_file():
        lines += ["## Navigation Hints", f"- Parent: [{parent.name}/]({parent_nav})"]

    return "\n".join(lines) + "\n"


def generate(directory: Path, dry_run: bool = False, force: bool = False) -> str:
    """Generate AGENTS.md navigation without overwriting authored guides."""
    if not directory.is_dir():
        print(f"  SKIP  {directory} (not a directory)", file=sys.stderr)
        return "failed"
    if directory.is_symlink() or (directory.name in SKIP_DIRS and not (directory / "build.gradle.kts").is_file()):
        return "skipped"
    ...
    if out_path.exists():
        try:
            is_generated = GENERATED_MARKER in out_path.read_text(errors="replace")
        except OSError:
            is_generated = False
        if not is_generated:
            return "skipped"
